...
export class SemesterPlanningService {
   ...
   */
  async getRecommendedSemesterPlan(studentId: number): Promise<SemesterPlanResult> {
    const key = `student:${studentId}:plan:recommended`
    return this.cache.wrap(key, 3 * 60 * 1000, async () => {
    this.logger.log(`Getting recommended semester plan for student ${studentId}`);
    ...
    this.logger.log(`Student ${student.student_code} study time: ${studyTimeHours} hours/week`);
    ...
    this.logger.log(`Found ${predictions.length} predicted courses for student ${studentId}`);
  ...
  async getSemesterPlanGrouped(studentId: number): Promise<{
    ...
    const semesters = Array.from(semesterMap.entries()).map(([key, courses]) => {
      ...
      const totalCredits = courses.reduce((sum, c) => sum + c.credits_unit, 0);
