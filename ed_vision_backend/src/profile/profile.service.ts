...
export class ProfileService {
	...
	async updateParentOccupation(
		accountId: number,
		data: { relationship_type?: string; occupation?: string; workplace?: string },
	) {
		const parent = await this.prisma.parent.findUnique({
			where: { account_id: accountId },
		});
	...
	async updateInstructorWork(accountId: number, updateDto: any) {
		...
		const data: any = {};
		if (updateDto.employee_code !== undefined) data.employee_code = updateDto.employee_code;
		if (updateDto.academic_title !== undefined) data.academic_title = updateDto.academic_title;
		if (updateDto.position !== undefined) data.position = updateDto.position;
		if (updateDto.department_id !== undefined) data.department_id = updateDto.department_id;
		if (updateDto.hire_date !== undefined) data.hire_date = updateDto.hire_date ? new Date(updateDto.hire_date) : null;
		...
		if (classes.length > 0) {
			await this.prisma.adviserAssignment.createMany({
				data: classes.map((cls: any) => ({
					...
					assigned_date: cls.assignedDate ? new Date(cls.assignedDate) : new Date(),
				})),
			});
		}
