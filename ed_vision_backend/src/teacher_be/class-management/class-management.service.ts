...
export class ClassManagementService {
  ...
  async getInstructorClasses(instructorId: number) {
    ...
    const classList = classGroups.map((classGroup) => {
      ...
      const teacherName =
        adviser?.account?.profile?.full_name || 'Not assigned';
  ...
  async getStudentsByClass(classCode: string) {
    ...
    return students.map((student) => {
      ...
      const gpa = hasCourseRecords
        ? this.calculateGPA(student.courseRecords)
        : null;
      const riskLevel =
        hasCourseRecords && gpa !== null ? this.determineRiskLevel(gpa) : null;
      ...
      return {
        ...
        status:
          hasCourseRecords && gpa !== null
            ? this.getStudentStatus(gpa)
            : 'No Data',
        riskLevel: riskLevel,
        address: student.account?.profile?.address || '',
        courses: student.courseRecords.length,
        totalCredits: student.courseRecords.reduce(
          (sum, record) => sum + (record.course?.credits_unit || 0),
          0,
        ),
      };
    });
  ...
  async uploadClassList(
    ...
    try {
      ...
      if (!students.length)
        throw new BadRequestException('No student data found');
      ...
      if (!classGroup) {
        ...
        if (!isAdviser)
          throw new BadRequestException('Access denied - not class adviser');
      }
      ...
      if (!studentRole)
        throw new BadRequestException('Student role not configured');
          ...
          if (!account.student) {
            await this.prisma.student.create({
              data: {
                ...
                cohort_year:
                  studentData.cohort_year || new Date().getFullYear(),
                class_id: classGroup.class_id,
                status: 'active',
              },
            });
            ...
            await this.prisma.student.update({
              ...
              data: {
                ...
                cohort_year:
                  studentData.cohort_year || account.student.cohort_year,
                class_id: classGroup.class_id,
              },
            });
            results.updated++;
   ...
   */
  private async parseClassFileNoValidation(
    file: Express.Multer.File,
  ): Promise<StudentRowData[]> {
    const rawData =
    ...
    const foundColumns = {
      studentCode: columns.find((col) =>
        requiredColumns.studentCode.includes(col),
      ),
    ...
    if (!foundColumns.studentCode)
      missingColumns.push('student_code (or Student Code, Mã SV, MSSV)');
    if (!foundColumns.fullName)
      missingColumns.push('full_name (or Full Name, Họ tên, Name)');
    if (!foundColumns.email)
      missingColumns.push('email (or Email, student_email)');
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      if (rowErrors.length > 0) {
        errors.push(
          `Row ${rowNum}: ${rowErrors.join(', ')} | Data: ${studentCode || 'N/A'} | ${fullName || 'N/A'} | ${email || 'N/A'}`,
        );
        continue;
      }
