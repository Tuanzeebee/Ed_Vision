...
export class AppointmentsService {
  ...
  async getAppointmentDashboard(
    ...
    return {
      summary: {
        ...
        weekAppointments: 28,
        pendingRequests: recentRequests.filter((a) => a.status === 'pending')
          .length,
  ...
  async getAppointments(
    ...
    if (filterDto.startDate) {
      filtered = filtered.filter(
        (a) => new Date(a.date) >= new Date(filterDto.startDate!),
      );
    }
    if (filterDto.endDate) {
      filtered = filtered.filter(
        (a) => new Date(a.date) <= new Date(filterDto.endDate!),
      );
    }
  ...
  async updateAppointmentStatus(
    ...
    // Mock implementation
    const appointment = await this.getAppointmentDetail(
      instructorId,
      appointmentId,
    );
    ...
    return {
      ...
      confirmedAt:
        dto.status === 'confirmed' ? new Date() : appointment.confirmedAt,
      completedAt:
        dto.status === 'completed' ? new Date() : appointment.completedAt,
    };
  }
  ...
  async getAvailabilityCalendar(
    ...
    return {
      month: new Date(year, month - 1).toLocaleString('vi-VN', {
        month: 'long',
      }),
      year,
      days,
  ...
  async getWeekOverview(
    ...
    const bookings = appointments.map((a: any) => {
      ...
      const parentDisplayName =
        a.appointmentContact?.contact_name ||
        a.booker?.profile?.full_name ||
        'Unknown';
      ...
      return {
        ...
        type: a.meeting_type === 'online' ? 'online' : 'offline',
        avatar,
      };
    });
