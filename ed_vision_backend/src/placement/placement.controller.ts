// ============================================================
// BƯỚC 4: src/placement/placement.controller.ts
// ============================================================
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!accountId || typeof accountId !== 'number') {
        return res
          .status(400)
          .json({ error: 'accountId (number) là bắt buộc' });
      }
      if (!Array.isArray(skillsToTest) || skillsToTest.length === 0) {
        return res.status(400).json({ error: 'skillsToTest[] là bắt buộc' });
      }
      const invalid = skillsToTest.filter(
        (s: string) => !VALID_SKILLS.includes(s),
      );
      if (invalid.length > 0) {
        return res
          .status(400)
          .json({ error: `Skill không hợp lệ: ${invalid.join(', ')}` });
      }
...
// Dùng khi user muốn bắt đầu lại hoặc session bị treo
placementRouter.post(
  '/abandon',
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      return res.status(200).json({ message: 'Session đã được huỷ.' });
    } catch (err) {
      next(err);
    }
  },
