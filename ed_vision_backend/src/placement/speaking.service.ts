...
export class SpeakingService {
  ...
  async transcribe(audioPath: string): Promise<string> {
    try {
      ...
      if (audioBuffer.length < 5000) {
        this.logger.warn(
          `[Speaking] Audio quá nhỏ (${audioBuffer.length} bytes) → coi như im lặng`,
        );
        return '';
      }
      ...
      const isSilence = silencePatterns.some((p) =>
      ...
      if (transcript.length < 3) {
        this.logger.warn(
          `[Speaking] Very short transcript (${transcript.length} chars)`,
        );
        return '[UNCLEAR_SPEECH]';
      }

      this.logger.log(
        `[Speaking] Transcript (${transcript.length} chars): ${transcript.slice(0, 100)}`,
      );
      return transcript;
    } catch (error: any) {
  ...
  async scoreSpeaking(
    ...
    if (!transcript || transcript.trim().length === 0 || wordCount === 0) {
      this.logger.warn(
        '[Speaking] Empty transcript → band 3.0 (no speech detected)',
      );
      try {
        return {
          band: 3.0,
              ...
              messages: [
                {
                  role: 'system',
                  content:
                    'You are an IELTS examiner. Return ONLY valid JSON, no markdown, no explanation outside JSON.',
                },
                {
                  role: 'user',
...
{"band": 5.5, "feedback": "Nhận xét ngắn bằng tiếng Việt tối đa 25 từ"}`,
        ...
        this.logger.log(
          `[Speaking] ${model} response: ${rawText.slice(0, 200)}`,
        );

        if (!res.ok) {
          this.logger.warn(
            `[Speaking] ${model} HTTP ${res.status} — trying next model`,
          );
          continue;
        }
        ...
        try {
          ...
          this.logger.warn(
            `[Speaking] Cannot parse JSON from ${model}: ${clean}`,
          );
          continue;
        }
      }
  ...
  private ruleBasedScore(
    ...
    if (
      ...
      return {
        band: 3.5,
        feedback:
          'Không thể phân tích âm thanh rõ ràng. Có thể do chất lượng mic hoặc tiếng ồn.',
      };
    }
    ...
    if (transcript === '[UNCLEAR_SPEECH]') {
      return {
        band: 3.5,
        feedback:
          'Phát hiện âm thanh nhưng không rõ ràng. Hãy nói to và rõ hơn.',
      };
    }
    ...
    if (wordCount === 0)
      return { band: 3.0, feedback: 'Không phát hiện giọng nói rõ ràng.' };
    if (wordCount < 5)
      return {
        band: 3.5,
        feedback: 'Câu trả lời quá ngắn. Hãy phát triển ý tưởng đầy đủ hơn.',
    ...
    const advancedFound = advancedWords.filter((w) =>
      transcriptLower.includes(w),
    );
  ...
  private extractKeywords(prompt: string): string[] {
    ...
    for (const [topic, words] of Object.entries(topicMap)) {
      if (
        ...
        words.some((w) => promptLower.includes(w))
      ) {
        return words;
      }
    }
