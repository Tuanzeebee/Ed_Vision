...
export class ChatService {
    ...
    if (recipientType === 'students' || recipientType === 'both') {
      for (const student of students) {
        try {
          ...
          console.error(
            `Failed to send to student ${student.student_id}:`,
            error,
          );
          results.failed++;
        }
  ...
  async sendQuickMessage(params: {
    ...
    const templates = {
      reminder:
        'Nhắc nhở: Bạn cần hoàn thành bài tập và tham gia đầy đủ các buổi học.',
      encouragement:
        'Cố lên! Thầy/Cô tin tưởng vào khả năng của em. Hãy tiếp tục nỗ lực!',
      concern:
        'Thầy/Cô nhận thấy em đang gặp khó khăn. Hãy liên hệ để được hỗ trợ.',
    };
  ...
  async sendUrgentAlert(params: {
    ...
    for (const studentId of params.studentIds) {
      try {
        ...
        if (params.notifyParents && student.parentStudentLinks) {
          for (const link of student.parentStudentLinks) {
            try {
              ...
              console.error(
                `Failed to send to parent ${link.parent?.parent_id}:`,
                error,
              );
              results.failed++;
            }
        ...
        console.error(
          `Failed to send urgent alert to student ${studentId}:`,
          error,
        );
        results.failed++;
      }
        ...
        for (const assignment of link.student.classGroup.adviserAssignments) {
          ...
          if (!teachersMap.has(teacherId)) {
            teachersMap.set(teacherId, {
              id: teacherId,
              name:
                assignment.instructor.account?.profile?.full_name || 'Unknown',
              avatar:
                assignment.instructor.account?.profile?.avatar_url || undefined,
              students: [],
            });
          }
