...
export class ParentChatController {
    ...
    async getConversations(@Request() req) {
        ...
        const parent = await this.chatService.getParentByAccountId(req.user.account_id);
        if (!parent) {
    ...
    async getMessages(
        ...
        return this.chatService.getMessages(conversationId, query.limit, query.skip);
    }
    ...
    async sendMessage(@Body() dto: SendMessageDto, @Request() req) {
        ...
        return this.chatService.sendMessage(dto.conversationId, senderId, 'parent', dto.content);
    }
    ...
    async createConversation(@Body() dto: { teacherId: string }, @Request() req) {
        ...
        const parentId = parent.parent_id.toString();
        return this.chatService.findOrCreateConversation(dto.teacherId, parentId, 'teacher-parent');
    }
}
