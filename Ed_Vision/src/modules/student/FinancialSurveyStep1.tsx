      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center px-4 pb-6">
          <div className="w-full max-w-md">
            {/* Instruction Text */}
            <p className="text-center text-gray-600 mb-4 text-sm leading-relaxed px-2">Trả lời một số câu hỏi nhanh để chúng tôi giúp bạn phân tích mức độ hỗ trợ tài chính từ gia đình một cách chính xác nhất.
            </p>
              ...
              <CardContent className="p-4 md:p-5">
                ...
                <div className="mb-4">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-1">
                    <div 
                      className="bg-[#5B4BDB] h-1 rounded-full transition-all duration-300"style={{ width: '25%'}}
                    />
                  </div>
                </div>

                {/* Question */}
                <h2 className="text-base md:text-ms font-bold text-gray-900 mb-4 leading-tight">Thu nhập hằng tháng trung bình của gia đình bạn khoảng bao nhiêu?
                </h2>
                    ...
                    <button
                      ...
                      className={`w-full p-2.5 md:p-3 text-left border rounded-md transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#5B4BDB] focus:ring-offset-1 text-sm font-medium cursor-pointer ${
                        selectedOption === option.value
                          ? 'border-[#5B4BDB] bg-[#F3F2FF] text-[#5B4BDB]': 'border-gray-200 text-gray-700 hover:border-gray-300 hover:shadow-sm'}`}
                      role="radio"aria-checked={selectedOption === option.value}
                      tabIndex={0}
                    >
                      {option.label}
                    </button>))}
                ...
                <div className="flex justify-end">
                  <Button
                    ...
                    className={`px-5 md:px-6 py-2 rounded-md font-semibold text-sm transition-all duration-200 ${
                      selectedOption
                        ? 'bg-[#5B4BDB] text-white hover:bg-purple-700 cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#5B4BDB] focus:ring-offset-2': 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
