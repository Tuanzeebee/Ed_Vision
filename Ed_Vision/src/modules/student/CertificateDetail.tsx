...
const practiceTests = getPracticeTests(cert.id)
  ...
  const realProgress = enrollment
    ...
    : cert.progress
  const realStatus: 'active' | 'not-started' | 'in-progress' | 'completed' = enrollment
    ...
    (topicKey: string) => {
      if (cert.id === 'toeic' && toeicProfile) {
        void saveToeicPlanSync({
          ...
          // Do not block navigation when sync is temporarily unavailable.
        })
      }
      const lessonPath = cert.id === 'toeic'
        ? `/student/certificate-review/${cert.id}/lesson/${topicKey}`
        : `/student/certificate-review/${cert.id}/lesson/${topicKey}${selectedBand ? `?band=${selectedBand}` : ''}`
      navigate(lessonPath)
    },
...
const bandOption = selectedBand ? getBandOption(cert.id, selectedBand) : undefined
  ...
  }, [isToeic, selectedBand, showFirstGuidePopup, toeicGuideCompleted, toeicProfile])
  ...
  const handleCloseGuide = useCallback(async () => {
    ...
    try {
      ...
      showErrorToast('Không thể lưu trạng thái hướng dẫn vào hệ thống. Vui lòng thử lại.')
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-5 sm:py-8 space-y-6 sm:space-y-8">
        ...
        {/* ── Cert Hero ── */}
        <section className={`bg-gradient-to-r ${cert.bgFrom} ${cert.bgTo} rounded-2xl p-4 sm:p-6 text-white`}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div
                className={`bg-white/20 rounded-2xl flex items-center justify-center shrink-0 ${
                  isToeic ? 'h-16 px-5 min-w-[108px]' : 'w-16 h-16'
                }`}
              >
                <span className={`font-black ${isToeic ? 'text-3xl leading-none tracking-wide' : 'text-2xl'}`}>{cert.icon}</span>
              </div>
              <div>
                <h1 className="text-2xl font-bold">{cert.label}</h1>
                <p className="text-white/70 text-sm mt-0.5">{cert.sublabel}</p>
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <span className="bg-white/20 px-2.5 py-1 rounded-lg text-sm font-bold">Tiến độ: {realProgress}%
                  </span>
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <div className="hidden sm:block min-w-[140px]">
                <div className="w-full bg-white/20 rounded-full h-2 mb-1.5">
                  <div
                    className="bg-white rounded-full h-2 transition-all"style={{ width: `${realProgress}%` }}
                  />
                </div>
              ...
              <button
                ...
                className="text-xs text-white/80 hover:text-white bg-white/10 hover:bg-white/20 px-2.5 py-1 rounded-lg transition-colors cursor-pointer">Đổi mục tiêu
              </button>
            </div>
          </div>
        </section>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-5 py-3.5 text-sm font-medium whitespace-nowrap transition-colors shrink-0 border-b-2 cursor-pointer ${
                        activeSkill === skill.id
                          ? 'border-purple-500 text-purple-700 bg-purple-50': 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}
                    >
                      <span
                        className={
                  ...
                  <div className="p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {activeSkillData.topics.map((topic, i) =>(
                        <div
                          key={i}
                          onClick={() => topic.topicKey && navigate(
                            cert.id === 'toeic'
                              ? `/student/certificate-review/${cert.id}/lesson/${topic.topicKey}`
                              : `/student/certificate-review/${cert.id}/lesson/${topic.topicKey}${selectedBand ? `?band=${selectedBand}` : ''}`,
                          )}
                          className={`flex items-start gap-2.5 p-3.5 rounded-xl border transition-all cursor-pointer hover:shadow-sm group ${
                            topic.done
                              ? 'bg-emerald-50 border-emerald-100 hover:border-emerald-300': 'bg-white border-slate-100 hover:border-purple-200 hover:bg-purple-50/30'}`}
                        >
                          <div
                    ...
                    <div className="mt-4 flex gap-2.5">
                      <button
                        onClick={() => {
                          ...
                          navigate(
                            cert.id === 'toeic'
                              ? `/student/certificate-review/${cert.id}/lesson/${firstTopic.topicKey}`
                              : `/student/certificate-review/${cert.id}/lesson/${firstTopic.topicKey}${selectedBand ? `?band=${selectedBand}` : ''}`,
                          )
                        }}
                      ...
                      <button
                        onClick={() => {
                          ...
                          navigate(
                            cert.id === 'toeic'
                              ? `/student/certificate-review/${cert.id}/lesson/${firstTopic.topicKey}?mode=flashcard`
                              : `/student/certificate-review/${cert.id}/lesson/${firstTopic.topicKey}${selectedBand ? `?band=${selectedBand}&mode=flashcard` : '?mode=flashcard'}`,
                          )
                        }}
                      ...
                      {isToeic ? (
                        <div className="space-y-4">
                          {skills.map((skill, idx) => {
                            ...
                            const icon = skill.id === 'listening' ? <Ear className="w-4 h-4" /> : <BookOpen className="w-4 h-4" />
                            const barColor = skill.id === 'listening' ? 'from-cyan-500 to-sky-500' : 'from-emerald-500 to-teal-500'
                            return (
                              <div key={skill.id} className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                                <div className="flex items-center justify-between text-sm mb-2">
                                  <span className="flex items-center gap-2 font-semibold text-slate-700">{icon}{skill.label}</span>
                                  <span className="font-bold text-slate-700">{value}/100</span>
                                </div>
                                <div className="h-2.5 w-full rounded-full bg-slate-200">
                                  <div className={`h-2.5 rounded-full bg-gradient-to-r ${barColor}`} style={{ width: `${Math.max(4, Math.min(100, value))}%` }} />
                                </div>
                                <p className="mt-2 text-xs text-slate-500">
                                  {value >= 75 ? 'Đang ổn định, tập trung tăng tốc độ.' : value >= 55 ? 'Mức trung bình, nên luyện đều mỗi ngày.' : 'Cần ưu tiên luyện để tránh mất điểm phần này.'}
                                </p>
                              </div>
                            )
                          })}
              ...
              <div className={`bg-gradient-to-r ${cert.bgFrom} ${cert.bgTo} rounded-2xl p-5 text-white mb-5`}>
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-white/80">Luyện tập các task MOS Word ngay trên trình duyệt. Hệ thống tự động chấm điểm theo tiêu chuẩn thi MOS.
                    </p>
                ...
                <div className="mt-4 flex items-center gap-3">
                  <div className="flex-1 bg-white/20 rounded-full h-2">
                    <div
                      className="bg-white rounded-full h-2 transition-all"style={{
                        width: `${
                          mosTasks.length >0
