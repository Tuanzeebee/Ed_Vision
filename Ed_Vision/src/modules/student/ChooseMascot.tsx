...
export default function ChooseMascot({ onConfirm, onSkip }: Props) {
  ...
  return (
    <div
      className="min-h-screen flex flex-col"style={{
      ...
      <header className="p-4 sm:p-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4 text-sm">
            <div
              className="px-3 py-1 rounded-full backdrop-blur-[20px] border border-white/20"style={{ background: "rgba(255, 255, 255, 0.6)"}}
            >
      ...
      <main className="flex-1 flex items-center justify-center px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto w-full">
          ...
          <div className="text-center mb-12">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-800 mb-4">Choose Your Learning Companion
            </h1>
            <p className="text-lg sm:text-xl text-gray-600 max-w-2xl mx-auto">Each mascot has a unique skill to support your journey.
            </p>
          </div>
          ...
          >
            <div
              className="inline-block px-6 py-3 rounded-2xl backdrop-blur-[20px] border border-white/20"style={{ background: "rgba(255, 255, 255, 0.6)"}}
            >
              <span className="text-purple-600 font-semibold flex items-center justify-center">
                <span className="mr-2"></span>Ready to begin your adventure!
          ...
          <div className="text-center">
            <Button
              ...
              className="text-white font-bold py-4 px-12 rounded-2xl text-lg mb-4 transition-all duration-300 hover:-translate-y-[2px] hover:shadow-[0_10px_25px_rgba(102,126,234,0.4)] disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none disabled:bg-gray-400"style={{
                background: selectedMascot
                  ? "linear-gradient(135deg, #667eea 0%, #764ba2 100%)": "#9CA3AF",
              }}
            >Confirm Mascot
            </Button>
            <div>
              <button
                onClick={handleSkip}
                className="text-gray-500 hover:text-gray-700 text-sm underline transition-colors">Skip for now
              </button>
            </div>
          </div>
        </div>
...
function MascotCard({ mascot, isSelected, onSelect }: MascotCardProps) {
  ...
  return (
    <div
      ...
      className="cursor-pointer transition-all duration-300 rounded-3xl"style={{
        background: isSelected
          ? "linear-gradient(135deg, rgba(124, 58, 237, 0.1), rgba(59, 130, 246, 0.1))": "rgba(255, 255, 255, 0.6)",
        transform:
          isHovered && !isSelected
            ? "translateY(-8px) rotateY(5deg)": isSelected
            ? "none": "translateY(0) rotateY(0)",
        boxShadow: isSelected
          ? "0 0 30px rgba(124, 58, 237, 0.4)": isHovered
          ? "0 20px 40px rgba(124, 58, 237, 0.2)": "none",
      }}
    >
      <Card
        className={`rounded-3xl p-6 text-center backdrop-blur-[20px] border bg-transparent ${
          isSelected
            ? "border-purple-600 border-2 animate-[sparkle_2s_infinite]": "border-white/20"}`}
      >
        <CardContent className="p-0">
        {/* Mascot Illustration */}
        <div
          className="w-[120px] h-[120px] rounded-full flex items-center justify-center text-5xl mx-auto mb-4 transition-transform duration-300"style={{
        ...
        <div className="flex items-center justify-center mb-3">
          ...
          <div
            className="w-8 h-8 rounded-full flex items-center justify-center"style={{ background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)"}}
          >
