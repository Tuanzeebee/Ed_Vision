...
export default function AmbiencePanel({
  ...
  return (
    <div
      className="fixed z-10"style={{ left: `${position.x}px`, top: `${position.y}px`, width: `${size.width}px` }}
    >
      <div
        className="backdrop-blur-[20px] bg-white/10 border border-white/20 rounded-3xl shadow-2xl flex flex-col"style={{ height: `${size.height}px` }}
      >
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-2.5 px-4 rounded-xl text-sm transition ${
                tab === 'sounds'? 'bg-gradient-to-r from-orange-500 to-amber-500 text-white font-semibold': 'bg-white/10 text-white/70 border border-white/20'}`}
            >Sounds
            </button>
            <button
              onClick={() =>setTab('animations')}
              className={`flex-1 py-2.5 px-4 rounded-xl text-sm transition ${
                tab === 'animations'? 'bg-gradient-to-r from-orange-500 to-amber-500 text-white font-semibold': 'bg-white/10 text-white/70 border border-white/20'}`}
            >Animations
            </button>
          </div>
        </div>
                    ...
                    <div
                      ...
                      <div
                        ...
                        className={`flex flex-col items-center justify-center w-16 h-16 rounded-2xl cursor-pointer transition-all duration-300 ${
                          isActive
                            ? 'bg-white/10': 'bg-white/5 text-white/50 hover:bg-white/10 hover:text-white/80'}`}
                        style={isActive ? {
                          color: activeColor,
                      ...
                      {/* Label */}
                      <span
                        className="text-xs font-medium uppercase tracking-wider"style={{ color: isActive ? activeColor : 'rgba(255,255,255,0.4)'}}
                      >
                        {sound.label}
                      </span>
                      ...
                      <input
                        ...
                        className="w-full h-1.5 rounded-full appearance-none cursor-pointer focus:outline-none ambience-slider"style={{
                          background: isActive 
          ...
          {tab === 'animations'&& (
            <>
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-white/5">
                  ...
                  <div
                    ...
                    className={`relative w-12 h-6 rounded-full cursor-pointer transition ${
                      showRain
                        ? 'bg-gradient-to-r from-orange-500 to-amber-500 shadow-[0_0_12px_rgba(249,115,22,0.5)]': 'bg-white/20'}`}
                  >
                    <div
              ...
              <button
                ...
                className="w-full py-3 px-4 rounded-xl bg-transparent border-2 border-orange-500/50 text-white font-medium hover:bg-orange-500/10 hover:border-orange-500 transition">RESET ALL
              </button>
            </>)}
        </div>
