...
export default function Sidebar({
  ...
  const normalizedPath = rawActive.startsWith("/")
    ? rawActive
    : `/teacher/${rawActive}`;

  // Map some short slugs to their actual route for menu matching
  const mappedPath =
    normalizedPath === "/teacher/management"
      ? "/teacher/appointments"
      : normalizedPath;

  const [isGradeMenuOpen, setIsGradeMenuOpen] = useState(
    mappedPath.includes("/teacher/grade") ||
      mappedPath.includes("/teacher/prediction") ||
      mappedPath.includes("/teacher/setting-grade-table"),
  );

  const [isAppointmentMenuOpen, setIsAppointmentMenuOpen] = useState(
    mappedPath.includes("/teacher/calendar") ||
      mappedPath.includes("/teacher/schedule") ||
      mappedPath.includes("/teacher/appointments"),
  );
  ...
  useEffect(() => {
    ...
    setIsGradeMenuOpen(
      p.startsWith("/teacher/grade") ||
        p.startsWith("/teacher/prediction") ||
        p.startsWith("/teacher/setting-grade-table"),
    );
    setIsAppointmentMenuOpen(
      p.startsWith("/teacher/calendar") ||
        p.startsWith("/teacher/schedule") ||
        p.startsWith("/teacher/appointments"),
    );
      ...
      <nav className="flex-1 p-4 space-y-1">
        ...
        <div className="space-y-1">
          <button
            ...
            className={`flex items-center justify-between px-3 py-2 rounded-lg w-full text-left transition-all duration-150 active:scale-95 cursor-pointer ${
              isGradeMenuOpen
          ...
          {isGradeMenuOpen && (
            <div className="ml-4 space-y-1">
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center space-x-3 px-4 py-2 rounded-lg w-full text-left transition-all duration-150 active:scale-95 cursor-pointer ${isActive ? "bg-blue-100 text-blue-700" : "text-gray-600 hover:bg-gray-50"}`
                }
              >
                <ClipboardList className="w-4 h-4" />
        ...
        <NavLink
          to="/teacher/exam-practice-import"
          className={({ isActive }) =>
            `flex items-center space-x-3 px-3 py-2 rounded-lg w-full text-left transition-all duration-150 active:scale-95 cursor-pointer ${isActive ? "bg-blue-50 text-blue-600" : "text-gray-700 hover:bg-gray-100"}`
          }
        >
          ...
          {isAppointmentMenuOpen && (
            <div className="ml-4 space-y-1">
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center space-x-3 px-4 py-2 rounded-lg w-full text-left transition-colors cursor-pointer ${isActive ? "bg-blue-100 text-blue-700" : "text-gray-600 hover:bg-gray-50"}`
                }
              >
                <Calendar className="w-4 h-4" />
